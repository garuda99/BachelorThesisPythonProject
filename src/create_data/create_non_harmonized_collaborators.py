...
# Enter the gathered Data into the AUTHOR_WORKS_WITH_AUTHOR_NON_HARMONIZED table
def create_non_harmonized_author_author_relations():
    ...
    with open(cleaned_data_path(), encoding="utf-8") as json_file:
        ...
        while line:
            ...
            for author in authors:
                ...
                if len(re.findall("[a-zA-Z]", unidecode(author))) > 2 and len(re.findall("[0-9]", author)) == 0:
                    new_authors.append(author)
            new_authors = sorted(new_authors)
        ...
        with connection:
            for author in author_dict.keys():
                ...
                sql_statement_first_half = """
                        INSERT INTO AUTHOR_WORKS_WITH_AUTHOR_NON_HARMONIZED (idOfAuthorOne, idOfAuthorTwo, numberOfPapers)
                        SELECT (SELECT a.id
                        FROM ALLAUTHORNAMES a WHERE a.name= ?),b.id,
                        CASE """
                ...
                connection.execute(sql_statement, params + params[1:])
        # The idOfAuthorOne should be smaller than idOfAuthorTwo
        # The entries that are the wrong way around will be flipped
        with connection:
            connection.execute("""INSERT INTO AUTHOR_WORKS_WITH_AUTHOR_NON_HARMONIZED (idOfAuthorTwo, idOfAuthorOne, numberOfPapers)
                SELECT * FROM
                AUTHOR_WORKS_WITH_AUTHOR_NON_HARMONIZED awwa 
        ...
        with connection:
            connection.execute("""DELETE
                FROM AUTHOR_WORKS_WITH_AUTHOR_NON_HARMONIZED 
